...
func GetUsers(w http.ResponseWriter, r *http.Request) {
	...
	result, err := db.Query("SELECT id, date, description, amount, status, receiver, jk, no_telp, address from transaksi_keuangan_risna")
	if err != nil {
		panic(err.Error())
	}
	defer result.Close()
...
func CreateUser(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	stmt, err := db.Prepare("INSERT INTO transaksi_keuangan_risna(date," +
...
func GetUser(w http.ResponseWriter, r *http.Request) {
	...
	result, err := db.Query("SELECT id, date,"+
		"description,amount,status,receiver,jk, no_telp,address from transaksi_keuangan_risna WHERE id = ?", params["id"])
	if err != nil {
		panic(err.Error())
	}
	defer result.Close()
...
func UpdateUser(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	params := mux.Vars(r)
	stmt, err := db.Prepare("UPDATE transaksi_keuangan_risna SET date= ?, description=?, amount=?, status=?, receiver=?, jk=?, no_telp=?, address=? WHERE id = ?")
...
func DeleteUser(w http.ResponseWriter, r *http.Request) {
	...
	stmt, err := db.Prepare("DELETE FROM transaksi_keuangan_risna WHERE id = ?")
	if err != nil {
		panic(err.Error())
	}
...
func InitDB() {
	db, err = sql.Open("mysql",
		"root:@tcp(127.0.0.1:3306)/db_2202648_risna_uas_b")
